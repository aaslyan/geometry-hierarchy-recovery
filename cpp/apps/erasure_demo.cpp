...
int main() {
  ...
  std::printf("hierarchy-erasure benchmark — build known hierarchy, flatten, "
              "strip, recover, score\n\n");
  std::printf("%-30s %6s %5s %5s %6s %5s %6s %8s  %s\n", "case", "rects", "cells",
              "lvls", "insts", "resid", "defct", "compress", "G⊆flat / leaf");
  for (const auto& c : cases) {
    ErasureDesign d = make_datapath(c.g, c.r, c.b, c.mirror, c.clutter, false);
    ErasureResult res = run_erasure(d);
    std::printf("%-30s %6zu %5d %5d %6d %5d %6d %7.2fx  %s / %s\n", c.label,
                d.flat.size(), res.cells, res.levels, res.top, res.residual,
                res.n_defective, res.compression, res.flatten_ok ? "YES" : "NO ",
                res.leaf_recovered ? "found" : "-");
  }
  std::printf("\nGround truth per case is the design's own construction "
              ...
              "decomposition — agreement, not accuracy (§1.3).\n");
  return 0;
}
