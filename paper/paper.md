...
Made quantitative over candidate motif length `L`, the signal is the **support** — the mean number of surviving copies per distinct motif [HR, §3]:

```
R(L)       = number of distinct motifs of size L that recur (≥ min copies)
F(L)       = total occurrences summed over those motifs
support(L) = F(L) / R(L)   — mean copies per surviving motif
```

`support(L)` is the clean "grow, grow, *drop*" curve. While the growing neighborhood is still inside the cell, one canonical motif explains nearly every instance, so `R` is small and support is high; the moment it grows past the cell into per-instance clutter it fragments into many motifs (`R` jumps while `F` holds), so support **falls off a cliff.** The cell scale is the size at the top of the sharpest *relative* support drop (scale-free). On the drop-off demo (`dropoff_demo`, a registered regression test) the cliff lands exactly on ground truth:

| Layout | Detected cell scale | Truth | Support drop |
|---|---|---|---|
| nested motif (scattered) | 2 | 2 | **8.0×** |
| defective motif (scattered) | 4 | 4 | 3.6× |
| dense datapath array | 4 | 4 | 2.0× |

The sharp cliff on the scattered motifs and the shallow one on the dense array are the honest boundary of the method: this is a **scale prior**, not a decision procedure. On a dense lattice sub-units repeat too, so the drop is weak — exactly why array detection instead reads the *period* (§3.1) and hierarchy relies on geometric verification and MDL gain to *certify* which candidates become cells (§5.4). The drop-off narrows *where to look*; it does not by itself certify a cell. We implement the curve as a first-class, inspectable analysis and expose promotion-at-the-drop-off as a selectable criterion, with MDL gain the default (§5.4).

The drop-off is only as faithful as the string it reads, and **physical size is what keeps it faithful.** Each motif token carries its rectangle's size, not just a symbol. Strip the size (a symbol-only ablation) and two motifs with the same *arrangement* but a differently-sized rectangle collapse into one: on a controlled layout of four `A` motifs and four `B` motifs (identical arrangement, `B`'s second rectangle wider), the size-aware signature keeps them apart (`R = 2`) while the size-blind one merges them (`R = 1`, support inflated 2×). This is the §5.1 edge-collision bug class turned into a positive check — symbol tells you *what pattern*, size tells you *which cell*, and the drop-off needs both.
...
**2D dense recovery: the lattice prior.** On the *full* 2D bitcell array, radius-based seed-and-extend growth returns nothing — the bitcell's own per-layer extent (≈ 4.2 µm diagonal on met1) *exceeds* the 3.95 µm vertical row pitch, so no growth radius separates a mirrored cell from its abutting vertical neighbor; every candidate over-grows and the exact-multiset gate (§5.4) reverts the round to zero cells. The uniform contact layer (licon) fails for a second reason: its rectangles are all identical squares, so the local geometric shingle (§5.4) is ambiguous — 1131 contacts collapse to five signature groups — the §4.4 lossiness from the adversarial side. Both failures are answered by letting the lattice *drive* candidate generation rather than only score it. `recover_lattice` finds a translational axis-aligned lattice `(dx, dy)` — candidate periods ranked by **self-similarity** (does translating by `d` map shapes onto identical shapes), which rejects the mirror pitch and the intra-tile spacings — extracts the fundamental-domain tile by **anchor-modulo** partition (each rectangle is assigned to the cell its min-corner lands in, so a tile that spills past its own cell is still separated, immune to extent > pitch), and tiles it with **count-aware** matching (real flattened geometry has coincident rectangles: SRAM bitlines and wordlines shared between abutting cells). Wired as a fallback when the shingle channel finds nothing, it recovers both stress layers: the met1 crop becomes a **5 × 5 array of a 128-rectangle tile** and the licon crop a 5 × 5 array of a 156-rectangle tile, each nested `gate → slice → block` with `flatten(H) = G` exact — where seed-and-grow recovered nothing.
...
**Full hierarchy extraction: a first nested extractor, with real limits.** Beyond the base recoverer's two compression views (placement-list and array-node), a first pass of the next step is now implemented as a separate `recover_nested` path: it promotes repeated placement runs into named intermediate `slice`/`block` cells whose bodies hold array references, recursing on the mixed representation (`leaf refs + array refs + residual`) under the same exact-multiset gate. On the synthetic datapaths it lifts the placement-list ~3.3–3.5× to a nested **9.5–14×** (`gate → slice → block → top`, exact), and on a real SKY130 SRAM row to **55×** (§7.6). The tile-extent-exceeds-pitch failure that once blocked 2D dense arrays is now handled by the **lattice prior** (§7.6): when seed-and-extend growth recovers nothing, `recover_lattice` drives candidate generation from the period instead — a translational lattice found by self-similarity, the fundamental-domain tile extracted by anchor-modulo, tiled with count-aware matching — and the SRAM's 2D met1 and licon crops recover exactly as nested `gate → slice → block` arrays. It is still **not full source-like extraction**. Two simplifications remain: nested cells are placed at orientation 0 with mirroring baked into distinct cell bodies (so a mirrored row is a second `slice` cell rather than one cell reused under D4), and the recovered 2D fundamental domain is the translational **2 × 2 super-cell** (§7.6) rather than a single bitcell factored into D4 orientations — which, with the shared-rail coincident geometry the prior cannot tile (≈ 40 % residual), holds the 2D compression to a modest ~2.3× even though the recovery is exact.
...
**Future work**, in rough priority order: (1) push the real-SKY130 evaluation further (§7.6) — the lattice prior now recovers the 2D array as a translational **2 × 2 super-cell**, so the next steps are factoring that super-cell into a single bitcell under D4, reducing the shared-rail residual (currently ≈ 40 %), and running an ORFS standard-cell crop plus the erasure harness; (2) **complete the nested extractor** — the §7.6 prototype recovers `slice`/`block` cells and nests exactly on synthetic and real layouts, but still needs D4-aware single-cell reuse for mirrored rows (rather than a second cell) and multi-layer tiles; (3) **tiling + parallelism** and streaming toward billion-polygon layouts; (4) **fractured-polygon verification** in the recovery path via reused canonicalization; (5) the **multi-serialization** run/substring generator of §4.2 and an occupancy-vs-edge ablation; (6) ILP/global selection to replace greedy; and (7) multi-layer support. Papers on either algorithm should follow real, benchmarked output — this draft is the working record toward that, not a finished result.
